...
#
# Sample script for AWS AutoScaling orchestration with CentrifyDC
#
#
        ...
        if ! aws --version ;then
            if ! python --version ;then
                ...
                rhel|amzn|centos)
                    ...
                    ;;
                ubuntu)
                    #Default Python source archives are old hence have to update for ubuntu 16.04
		    case "$OS_VERSION" in
		    16|16*)
		        apt-get update
			;;
...
{
	rm -f /usr/share/centrifydc/bin/getkeytab.sh
    if [ "$CENTRIFYDC_USE_CUSTOM_KEYTAB_FUNCTION" = "yes" ]; then
		if [ "`type -t $CENTRIFYDC_CUSTOM_KEYTAB_FUNCTION`" == "function" ]; then
			echo '#!/bin/bash' > /usr/share/centrifydc/bin/getkeytab.sh
			declare -f $CENTRIFYDC_CUSTOM_KEYTAB_FUNCTION >> /usr/share/centrifydc/bin/getkeytab.sh
			echo $CENTRIFYDC_CUSTOM_KEYTAB_FUNCTION >> /usr/share/centrifydc/bin/getkeytab.sh
		else
		...
		echo '#!/bin/bash' > /usr/share/centrifydc/bin/getkeytab.sh
		echo aws s3 cp s3://$CENTRIFYDC_KEYTAB_S3_BUCKET/login.keytab $centrifydc_deploy_dir/ >> /usr/share/centrifydc/bin/getkeytab.sh
	fi
	chmod 500 /usr/share/centrifydc/bin/getkeytab.sh
	
	/usr/share/centrifydc/bin/getkeytab.sh
	r=$?
	
	if [ $r -ne 0 ]; then
		echo "$CENTRIFY_MSG_PREX: cannot download login.keytab file"
		return 2
	fi
	if [ ! -f $centrifydc_deploy_dir/login.keytab ]; then
		echo "$CENTRIFY_MSG_PREX: login.keytab is not found.  Possibly not set up correctly in /usr/share/centrifydc/bin/getkeytab.sh"
		return 2
	fi
